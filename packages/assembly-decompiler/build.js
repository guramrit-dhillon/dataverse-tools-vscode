#!/usr/bin/env node
/**
 * Build script for the AssemblyDecompiler .NET backend.
 *
 * Usage:
...
const CSPROJ = path.join(DOTNET_DIR, "AssemblyDecompiler.csproj");
...
function buildForRid(rid) {
  ...
  console.log(`[decompiler] Building ${rid}...`);
  execSync(
  ...
  console.log(`[decompiler]   -> ${outputDir}`);
}

// ── Main ──────────────────────────────────────────────────────────────────────

if (!hasDotnet()) {
  console.warn("[decompiler] dotnet SDK not found — skipping .NET build");
  process.exit(0);
}
...
console.log("[decompiler] Build complete.");
